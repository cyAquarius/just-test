 ...
 * 以及 {@code @SmartTestProject} 声明的兼容别名。
 *
...
public class SmartTestProjectDataSourceAliasPostProcessor implements BeanDefinitionRegistryPostProcessor {
    ...
    static final String DATA_SOURCE_BEAN = "smartTestDataSource";
    static final String TRANSACTION_MANAGER_BEAN = "smartTestTransactionManager";
    ...
    private static final Logger log = LoggerFactory.getLogger(SmartTestProjectDataSourceAliasPostProcessor.class);

    private final String[] extraDataSourceAliases;
    private final String[] extraTransactionManagerAliases;

    public SmartTestProjectDataSourceAliasPostProcessor() {
        this(new String[0], new String[0]);
    }

    public SmartTestProjectDataSourceAliasPostProcessor(String[] extraDataSourceAliases,
    ...
    static void register(BeanDefinitionRegistry registry,
        ...
        RootBeanDefinition definition = new RootBeanDefinition(SmartTestProjectDataSourceAliasPostProcessor.class);
    ...
    static void aliasIfAbsent(BeanDefinitionRegistry registry, String canonicalName, String alias) {
        ...
        if (registry.containsBeanDefinition(trimmedAlias)) {
            log.info("[SmartTest] Skipping alias '{}' for '{}': a bean definition with that name already exists",
                    trimmedAlias, canonicalName);
            return;
        }
        if (registry.isAlias(trimmedAlias)) {
            log.info("[SmartTest] Skipping alias '{}' for '{}': an alias with that name already exists",
                    trimmedAlias, canonicalName);
            return;
        }
