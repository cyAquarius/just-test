...
import com.just.test.internal.datasource.SchemaInitializer;
import com.just.test.internal.datasource.JustTestRoutingDataSource;
import com.just.test.internal.loader.DataSetLoader;
import com.just.test.internal.mock.JustMockInjector;
import com.just.test.mock.StaticMockContext;
import com.just.test.internal.mock.ThreadScopedMockRegistry;
import com.just.test.internal.scope.ThreadScope;
import com.just.test.internal.verifier.DataSetVerifier;
import com.just.test.internal.verifier.ExceptionVerifier;
import com.just.test.internal.verifier.ResultVerifier;
 ...
 *
 * <p>不对消费方提供兼容承诺。由内部 test-template provider 注册为 JUnit 扩展，
 * 因此必须保持 public。</p>
 *
 ...
 */
public final class JustTestExtension implements BeforeEachCallback, AfterEachCallback,
    ...
    private JustTestRoutingDataSource routingDataSource;
    private StaticMockContext staticMockContext;
    private JdbcTemplate jdbcTemplate;
    private JustTestLifecycle lifecycle;
    private boolean caseBound;

    public JustTestExtension(CaseContext caseContext) {
        if (caseContext == null) {
    ...
    public void beforeEach(ExtensionContext extensionContext) throws Exception {
        ...
        if (!(testInstance instanceof JustTestLifecycle)) {
            throw new IllegalStateException(String.format(
                    "[JustTest] %s must implement JustTestLifecycle",
                    testInstance.getClass().getName()));
        }
        lifecycle = (JustTestLifecycle) testInstance;
        ...
        try {
            ...
            JustMockInjector.injectJustMocks(testInstance, applicationContext);
    ...
    public void interceptTestTemplateMethod(Invocation<Void> invocation,
        ...
        try {
            ...
            try {
                ...
                log.debug("[JustTest] Exception captured for case [{}]: {}",
    ...
    private void prepareCaseData() {
        ...
        log.debug("[JustTest] Data prepared for case: {}", caseContext.getCaseName());
    }
    ...
    private void verifyCase() throws Throwable {
        ...
        if (!failures.isEmpty()) {
            StringBuilder message = new StringBuilder("[JustTest] Verification failed for case [")
        ...
        log.debug("[JustTest] Verification passed for case: {}", caseContext.getCaseName());
    }
    ...
    private void warnIfMultipleContexts(Class<?> testClass) {
        if (CONTEXT_DIAGNOSTICS.markRiskWarning()) {
            log.warn("[JustTest] Multiple Spring ApplicationContexts detected for {}. "
    ...
    private void warnFailure(ExtensionContext extensionContext, Throwable failure) {
        ...
        if (CONTEXT_DIAGNOSTICS.markFailureWarningFor(testClass)) {
            log.warn("[JustTest] Case [{}] failed with {} while multiple Spring ApplicationContexts "
    ...
    private JustTestRoutingDataSource beginCaseDatabase(ApplicationContext context) {
        JustTestRoutingDataSource dataSource = requireRoutingDataSource(context);
        dataSource.beginCase();
        return dataSource;
    }

    /**
     * 解析框架拥有的 {@link JustTestRoutingDataSource}；缺失视为配置错误。
     */
    static JustTestRoutingDataSource requireRoutingDataSource(ApplicationContext applicationContext) {
        try {
            return applicationContext.getBean(JustTestRoutingDataSource.class);
        } catch (NoSuchBeanDefinitionException e) {
            throw new IllegalStateException(
                    "[JustTest] JustTestRoutingDataSource is missing. "
                            + "@JustTest owns H2 infrastructure and requires the framework routing DataSource; "
    ...
    static JdbcTemplate requireJdbcTemplate(ApplicationContext applicationContext) {
        try {
            ...
            throw new IllegalStateException(
                    "[JustTest] JdbcTemplate is missing. "
                            + "@JustTest owns H2 infrastructure and requires the framework JdbcTemplate; "
                            + "this is a configuration error.", e);
        }
    }
    ...
    private void prewarmThreadScopedMocks(ApplicationContext context) {
        ...
        for (String beanName : registry.getBeanNames()) {
            try {
                ...
                log.warn("[JustTest] Failed to prewarm thread-scoped mock '{}': {}", beanName, e.getMessage());
            }
        }
    }
    ...
    private void invokeBeforeCaseMethod(Object testInstance, Method method, String caseName) {
        try {
            ...
            log.debug("[JustTest] Invoked @BeforeCase(\"{}\") -> {}", caseName, method.getName());
        } catch (Exception e) {
            Throwable cause = e.getCause() != null ? e.getCause() : e;
            throw new RuntimeException(String.format(
                    "[JustTest] @BeforeCase(\"%s\") method %s failed: %s",
