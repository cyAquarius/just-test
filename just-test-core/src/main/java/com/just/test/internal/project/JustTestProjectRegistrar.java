 ...
 */
public class JustTestProjectRegistrar implements ImportBeanDefinitionRegistrar,
        EnvironmentAware, ResourceLoaderAware, BeanClassLoaderAware {

    static final String ANNOTATION_NAME = "com.just.test.annotation.JustTestProject";
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata,
        ...
        scanner.addExcludeFilter(new JustTestProjectTypeExcludeFilter(
                ...
                classLoader));
        JustTestProjectScanFilters.apply(scanner, attributes, environment, resourceLoader, registry, classLoader);
        scanner.scan(basePackages);

        if (mapperPackages.length > 0) {
            JustTestProjectMyBatisSupport.register(registry, mapperPackages);
        }
    ...
    static AnnotationAttributes resolveAttributes(AnnotationMetadata metadata) {
        ...
        if (attributes == null) {
            throw new IllegalStateException(
                    "[JustTest] @JustTestProject metadata is missing on " + metadata.getClassName());
        }
        return attributes;
    }
    ...
    static String[] requiredBasePackages(AnnotationAttributes attributes) {
        ...
        if (basePackages.length == 0) {
            throw new IllegalStateException(
                    "[JustTest] @JustTestProject requires basePackages (the application root to scan). "
                            + "JustTest does not guess a business package; declare it explicitly, "
    ...
    static void requireMyBatisSpringIfNeeded(String[] mapperPackages, ClassLoader classLoader) {
        ...
        if (!ClassUtils.isPresent(MYBATIS_SPRING_FACTORY, classLoader)) {
            throw new IllegalStateException(
                    "[JustTest] @JustTestProject mapperPackages requires mybatis-spring on the test "
