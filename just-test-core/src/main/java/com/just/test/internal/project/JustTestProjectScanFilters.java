...
/**
 * 将 {@code @JustTestProject} 上的 include/exclude filters 应用到扫描器。
 *
 * <p>用户过滤器叠加在默认 denylist 之上，不替换框架默认排除。</p>
 */
final class JustTestProjectScanFilters {

    private JustTestProjectScanFilters() {
    }
        ...
        if (filterType == FilterType.ANNOTATION) {
            ...
            if (!TypeFilter.class.isAssignableFrom(filterClass)) {
                throw new IllegalStateException(
                        "[JustTest] @JustTestProject custom filter must implement TypeFilter: "
                                + filterClass.getName());
            }
            typeFilter = instantiateCustomFilter(filterClass, environment, resourceLoader, registry);
        } else {
            throw new IllegalStateException(
                    "[JustTest] @JustTestProject filter type " + filterType
