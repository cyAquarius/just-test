...
**为什么要简单类名这一层：** 同一包下可以有多个 `@SmartTest` 类。若 YAML 直接堆在包目录，发现器会拾取兄弟类的用例，造成串 case。类名目录把每个测试类的 YAML 隔开。

**包级回退是遗留兼容，不是推荐路径。** 默认类名目录不存在时，框架会扫测试类同包目录并打 **WARN**（可能拾取同包其他类的 YAML）。新用例不要依赖这个回退。

**逃生口：** `@CaseSource("custom-root")` 使用测试类包下的自定义根，例如 `src/test/resources/com/example/smarttest/order/custom-root/<case-name>/`。自定义根仍应按类或职责隔离，不要把多个类的 case 倒进同一个无差别目录。
...
- 把多个类的 YAML 倒进同一包级目录，或依赖包级 WARN 回退。
