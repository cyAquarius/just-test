...
- 框架代码统一放在 `com.just.test.smarttest` 下。稳定公开 API 仅限 `annotation`、`CaseContext`、`SmartTestLifecycle`、`StaticMockContext`，以及 Boot 模块的 `@SmartTest` / `@SmartTestProject`；`SmartTestClassValidationExtension` 由 `@SmartTest` 元注解注册，勿直接依赖。引擎实现放在 `com.just.test.smarttest.internal`，不对消费方提供兼容承诺。
